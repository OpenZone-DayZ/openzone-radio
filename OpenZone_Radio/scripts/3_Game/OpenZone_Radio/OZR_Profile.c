...
//
// НАПОВНЮЄТЬСЯ З ПАКЕТА СИНХРОНІЗАЦІЇ ЯДРА (OZR_ClientSync, з 2026-09-20), і
// історія тут повчальна. Перший варіант возив один рядок власним RPC -- сітку
// й усі профілі разом, -- і на одинадцятому профілі він переріс межу рушія:
// рядок-значення ріжеться на 1023 байтах, а обробник падає з «String
// CORRUPTED - FIX OnStoreLoad()», ТИХО з точки зору гравця. Другий возив числа:
// сітка трьома, профіль чотирма, кожен своїм пакетом. Пакет ядра тієї межі не
// має -- OZ_Sync ріжеться на частини й склеюється до розбору, -- тож профілі
// знову їдуть одним об'єктом; SetGrid і AddProfile лишились окремими кроками,
// бо порядок застосування той самий: сітка скидає перелік, профілі йдуть за нею.
class OZR_ClientGrid
{
    ...
    // сервер більше й не надсилає нерівну -- див. OZR_EtherServer.Fill.
